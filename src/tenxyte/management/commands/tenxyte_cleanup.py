...
from tenxyte.models import BlacklistedToken, MagicLinkToken, OTPCode
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--dry-run', action='store_true', help="Ne supprime rien, affiche juste ce qui serait supprimé")
        parser.add_argument('--login-attempts-days', type=int, default=90, help="Jours avant suppression des LoginAttempt (0 = garder)")
        parser.add_argument('--audit-log-days', type=int, default=365, help="Jours avant suppression des AuditLog (0 = garder)")
    ...
    def handle(self, *args, **options):
        ...
        if login_days > 0:
            try:
                ...
                if not dry_run:
                    qs.delete()
                self.stdout.write(self.style.SUCCESS(f'Login attempts (older than {login_days} days): {la_count} deleted.'))
            except Exception as e:
