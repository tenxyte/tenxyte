...
|----|----------|------|-----------|---------------|----------|---------------------|
| MT-1 | Matrice d'installation | 2026-08-28 | Claude (agent) | dev (sous-processus, pas de venv réel) | ⚠️ Partiellement vérifié | Comportement du code validé (import sans Django, erreur explicite, transparence avec Django) via `tests/core/test_import_guard.py` + `tests/core/test_packaging.py` (Property 12) ; job CI `install-matrix` ajouté (`.github/workflows/ci.yml`) pour la procédure `venv` réelle des 4 cibles — reste à exécuter en CI/manuellement pour clôturer MT-1 formellement |
| MT-2 | Release 0.9.x d'avertissement | — | — | — | ⬜ À exécuter | `DeprecationWarning` implémenté et testé (`tenxyte/__init__.py`, self-disabling) ; publication réelle d'une release 0.9.x intermédiaire = décision/action mainteneur |
| MT-3 | Apple Sign-In E2E (scénarios 3.1–3.7) | — | — | — | ⬜ À exécuter | Nécessite un compte Apple Developer réel ; logique unitairement + property-testée (mocks) dans `tests/integration/django/unit/test_social_auth.py` et `test_apple_oauth_properties.py` |
| MT-4 | GitHub PVR | — | — | — | ⬜ À exécuter | Action admin GitHub hors de portée de l'agent |
| MT-5 | Trusted Publishing | — | — | — | ⬜ À exécuter | TestPyPI d'abord ; action owner PyPI + admin GitHub hors de portée de l'agent |
| MT-6 | Attestations | — | — | — | ⬜ À exécuter | Dépend de MT-5 |
| MT-7 | Revue documentaire | 2026-08-28 | Claude (agent) | — | ⚠️ PASS avec réserve | SECURITY.md, stability.md (EN/FR), docs/security-audit/*, README EN/FR à jour ; `validate_endpoints.py` : 0 erreur EN, 4 erreurs FR **pré-existantes** (non liées à Apple — mêmes erreurs présentes avant cette phase, cf. session) à corriger séparément |
