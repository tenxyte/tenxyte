...
- [x] Tous les tests existants passent sans modification (non-régression, Requirement 8) — suite
      complète : 2647 passed, 0 failed, 8 skipped (voir `tasks.md` tâche 10.1).
- [x] `pip install tenxyte` dans un venv vierge sans Django : `import tenxyte` fonctionne, version
      core utilisable — vérifié par simulation sous-processus (`tests/core/test_import_guard.py`) ;
      job CI `install-matrix` ajouté pour la vérification en venv réel (reste à exécuter, MT-1).
- [x] `pip install tenxyte[django]` : comportement identique à l'actuel `pip install tenxyte` —
      équivalence d'ensemble de dépendances vérifiée (`tests/core/test_packaging.py`, Property 12).
- [x] `SECURITY.md` publié. ⬜ Rendu par GitHub (onglet Security) — nécessite l'activation manuelle
      de Private Vulnerability Reporting (MT-4, droits admin GitHub requis).
- [x] `publish.yml` utilise Trusted Publishing + génère des attestations (code prêt). ⬜
      Configuration effective du Trusted Publisher PyPI et de l'environnement GitHub protégé
      (MT-5, droits owner PyPI requis).
- [ ] Login Apple E2E validé manuellement (voir `manual_tests.md` §3, MT-3) — nécessite un compte
      Apple Developer réel ; logique unitairement + property-testée (mocks) côté code.
- [x] `docs/en/stability.md` + `docs/fr/stability.md` publiés.
- [x] Modèle de menaces + périmètre d'audit livrés dans `docs/security-audit/`.
- [x] `pyproject.toml` : version `1.0.0`, classifieur `Development Status :: 5 - Production/Stable`.
- [x] CHANGELOG 1.0.0 + guide de migration 0.9 → 1.0 publiés.

**Statut global : implémentation et documentation complètes ; le tag `v1.0.0` reste bloqué sur les
tests manuels externes (MT-2 à MT-6) qui nécessitent des accès (Apple Developer, admin GitHub,
owner PyPI) hors de portée de cette session — voir `manual_tests.md` pour le détail.**
