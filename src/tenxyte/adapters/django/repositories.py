...
class DjangoUserRepository(UserRepository):
    ...
    def _extract_metadata(self, django_user) -> Dict[str, Any]:
        ...
        optional_fields = [
            'phone_country_code', 'phone_number', 'google_id',
            'backup_codes', 'is_banned', 'is_locked', 'is_deleted',
...
class DjangoOrganizationRepository(OrganizationRepository):
    ...
    def list_by_user(
        ...
        org_ids = MembershipModel.objects.filter(
            user_id=user_id,
            status='active'
        ).values_list('organization_id', flat=True)
        
        # Get organizations
        orgs = OrganizationModel.objects.filter(
            id__in=org_ids,
            is_active=True
        )[skip:skip + limit]
    ...
    def get_children(self, org_id: str) -> List[Organization]:
        ...
        children = OrganizationModel.objects.filter(
            parent_id=org_id,
            is_active=True
        )
    ...
    def remove_member(self, org_id: str, user_id: str) -> bool:
        ...
        try:
            membership = MembershipModel.objects.get(
                organization_id=org_id,
                user_id=user_id
            )
            membership.delete()
