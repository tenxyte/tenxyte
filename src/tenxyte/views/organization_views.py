...
@extend_schema(
    ...
    request=inline_serializer(
        ...
        fields={
            ...
            "slug": serializers.CharField(
                required=False, allow_blank=True, help_text="Slug unique (optionnel, généré automatiquement)"
            ),
            "description": serializers.CharField(required=False, allow_blank=True, help_text="Description (optionnel)"),
            "parent_id": serializers.IntegerField(
                required=False, allow_null=True, help_text="ID organisation parent (optionnel)"
            ),
            "metadata": serializers.DictField(
                required=False, allow_null=True, help_text="Métadonnées personnalisées (optionnel)"
            ),
            "max_members": serializers.IntegerField(
                required=False, default=0, help_text="Limite de membres (0 = illimité)"
            ),
        },
    ),
    responses={
    ...
    examples=[
        ...
        OpenApiExample(
            ...
            value={"name": "Acme France", "slug": "acme-france", "parent_id": 1, "description": "Filiale française"},
        ),
        OpenApiExample(
            response_only=True,
...
def create_organization(request: Request) -> Response:
    ...
    return Response(
        OrganizationSerializer(organization, context={"request": request}).data, status=status.HTTP_201_CREATED
    )
...
@extend_schema(
    ...
    parameters=[
        ...
        OpenApiParameter(
            name="page", type=OpenApiTypes.INT, location=OpenApiParameter.QUERY, description="Numéro de page"
        ),
        OpenApiParameter(
            name="page_size",
    ...
    examples=[
        OpenApiExample(
            ...
            value={
                ...
                "previous": None,
                "results": [{"id": 1, "name": "Acme Corp", "slug": "acme-corp", "member_count": 15, "is_active": True}],
            },
        )
    ],
)
...
@extend_schema(
    ...
    examples=[
        ...
        OpenApiExample(
            ...
            name="org_not_member",
            summary="Non-membre",
            value={"error": "User is not a member of this organization", "code": "NOT_MEMBER"},
        ),
    ],
)
@api_view(["GET"])
...
@extend_schema(
    ...
    request=inline_serializer(
        ...
        fields={
            ...
            "max_members": serializers.IntegerField(
                required=False, allow_null=True, help_text="Nouvelle limite membres"
            ),
    ...
    examples=[
        OpenApiExample(
            ...
            value={"name": "Acme Corporation", "description": "Mise à jour description", "max_members": 200},
        ),
        OpenApiExample(
            response_only=True,
            name="parent_constraint_violation",
            summary="Violation contrainte parent",
            value={"error": "Cannot set parent: would create circular hierarchy", "code": "CIRCULAR_HIERARCHY"},
        ),
        OpenApiExample(
            response_only=True,
            name="member_limit_too_low",
            summary="Limite membres trop basse",
            value={"error": "Member limit cannot be less than current member count", "code": "MEMBER_LIMIT_VIOLATION"},
        ),
    ],
)
@api_view(["PATCH"])
...
def update_organization(request: Request) -> Response:
    ...
    success, error = service.update_organization(
        organization=request.organization, user=request.user, **serializer.validated_data
    )
    ...
    return Response(
        OrganizationSerializer(request.organization, context={"request": request}).data, status=status.HTTP_200_OK
    )
...
@extend_schema(
    ...
    examples=[
        OpenApiExample(response_only=True, name="delete_org_success", summary="Suppression réussie", value=None),
        OpenApiExample(
            response_only=True,
            name="has_child_organizations",
            summary="Organisations enfants présentes",
            value={"error": "Cannot delete organization with child organizations", "code": "HAS_CHILDREN"},
        ),
        OpenApiExample(
            response_only=True,
            name="not_owner",
            summary="Pas propriétaire",
            value={"error": "Only organization owners can delete organizations", "code": "NOT_OWNER"},
        ),
    ],
)
@api_view(["DELETE"])
...
def delete_organization(request: Request) -> Response:
    service = OrganizationService()
    success, error = service.delete_organization(organization=request.organization, user=request.user)
...
@extend_schema(
    ...
    examples=[
        OpenApiExample(
            ...
            value={"user_id": 123, "role_code": "member"},
        ),
        OpenApiExample(
            response_only=True,
            name="member_limit_exceeded",
            summary="Limite de membres dépassée",
            value={"error": "Organization member limit exceeded", "code": "MEMBER_LIMIT_EXCEEDED"},
        ),
        OpenApiExample(
            response_only=True,
            name="already_member",
            summary="Déjà membre",
            value={"error": "User is already a member of this organization", "code": "ALREADY_MEMBER"},
        ),
        OpenApiExample(
            response_only=True,
            name="invalid_role",
            summary="Rôle invalide",
            value={"error": "Cannot assign owner role through add_member", "code": "INVALID_ROLE"},
        ),
    ],
)
@api_view(["POST"])
...
@extend_schema(
    ...
    request=inline_serializer(
        ...
        fields={"role_code": serializers.CharField(help_text="Nouveau rôle (owner nécessite droits appropriés)")},
    ),
    responses={
        200: {
            "type": "object",
    ...
    examples=[
        OpenApiExample(
            response_only=True, name="update_role_success", summary="Rôle mis à jour", value={"role_code": "admin"}
        ),
        OpenApiExample(
            response_only=True,
            name="cannot_demote_owner",
            summary="Impossible de rétrograder owner",
            value={"error": "Cannot demote organization owner", "code": "CANNOT_DEMOTE_OWNER"},
        ),
        OpenApiExample(
            response_only=True,
            name="last_owner_error",
            summary="Dernier propriétaire",
            value={"error": "Cannot remove role from last organization owner", "code": "LAST_OWNER_REQUIRED"},
        ),
    ],
)
@api_view(["PATCH"])
...
@extend_schema(
    ...
    examples=[
        OpenApiExample(
            response_only=True, name="remove_member_success", summary="Membre retiré avec succès", value=None
        ),
        OpenApiExample(
            response_only=True,
            name="cannot_remove_owner",
            summary="Impossible de retirer owner",
            value={"error": "Cannot remove organization owner", "code": "CANNOT_REMOVE_OWNER"},
        ),
        OpenApiExample(
            response_only=True,
            name="last_owner_error",
            summary="Dernier propriétaire",
            value={"error": "Cannot remove last organization owner", "code": "LAST_OWNER_REQUIRED"},
        ),
        OpenApiExample(
            response_only=True,
            name="self_removal",
            summary="Auto-suppression",
            value={"message": "Member removed successfully"},
        ),
    ],
)
@api_view(["DELETE"])
...
def remove_member(request: Request, user_id: int) -> Response:
    ...
    success, error = service.remove_member(
        organization=request.organization, user_to_remove=user_to_remove, removed_by=request.user
    )
...
@extend_schema(
    ...
    request=inline_serializer(
        ...
        fields={
            ...
            "expires_in_days": serializers.IntegerField(
                required=False, default=7, min_value=1, max_value=30, help_text="Durée de validité en jours (1-30)"
            ),
        },
    ),
    responses={
    ...
    examples=[
        OpenApiExample(
            ...
            value={"email": "new.user@example.com", "role_code": "member", "expires_in_days": 7},
        ),
        OpenApiExample(
            response_only=True,
            name="already_member",
        ...
        OpenApiExample(
            ...
            value={"error": "Pending invitation already exists for this email", "code": "INVITATION_EXISTS"},
        ),
    ],
)
@api_view(["POST"])
            ...
            value=[
                {
                    ...
                    "weight": 100,
                    "permissions": [{"code": "org.*", "name": "All organization permissions"}],
                    "is_system_role": True,
                },
                {
                    "code": "admin",
                ...
                },
                {
                    ...
                    "weight": 10,
                    "permissions": [{"code": "org.read", "name": "View organization"}],
                    "is_system_role": True,
                },
            ],
        )
