...
class WebAuthnService:
    ...
    def begin_registration(self, user) -> Tuple[bool, Optional[Dict[str, Any]], str]:
        ...
        challenge_instance, raw_challenge = WebAuthnChallenge.generate(
            operation=WebAuthnChallenge.OPERATION_REGISTER, user=user, expiry_seconds=300
        )
        ...
        try:
            options = webauthn.generate_registration_options(
                ...
                user_display_name=f"{getattr(user, 'first_name', '')} {getattr(user, 'last_name', '')}".strip()
                or user.email,
                challenge=raw_challenge.encode(),
                exclude_credentials=[
                    webauthn.PublicKeyCredentialDescriptor(id=cid.encode()) for cid in existing_credentials
                ],
            )
            return (
                True,
    ...
    def complete_registration(
        self, user, credential_data: Dict[str, Any], challenge_id: int, device_name: str = ""
        ...
        try:
            challenge_instance = WebAuthnChallenge.objects.get(
                id=challenge_id, user=user, operation=WebAuthnChallenge.OPERATION_REGISTER
            )
    ...
    def begin_authentication(self, user=None) -> Tuple[bool, Optional[Dict[str, Any]], str]:
        ...
        challenge_instance, raw_challenge = WebAuthnChallenge.generate(
            operation=WebAuthnChallenge.OPERATION_AUTHENTICATE, user=user, expiry_seconds=300
        )
    ...
    def complete_authentication(
        ...
        try:
            challenge_instance = WebAuthnChallenge.objects.get(
                id=challenge_id, operation=WebAuthnChallenge.OPERATION_AUTHENTICATE
            )
        except WebAuthnChallenge.DoesNotExist:
            return False, None, "Invalid or expired challenge"
        ...
        try:
            stored_credential = WebAuthnCredential.objects.select_related("user").get(credential_id=raw_credential_id)
