...
class AuditLogListView(APIView):
    ...
    @extend_schema(
        ...
        parameters=[
            ...
            OpenApiParameter(
                "action", str, description="Filtrer par action (login, login_failed, password_change, etc.)"
            ),
...
class AuditLogDetailView(APIView):
    ...
    def get(self, request, log_id):
        try:
            ...
            return Response({"error": "Audit log not found", "code": "NOT_FOUND"}, status=status.HTTP_404_NOT_FOUND)
...
class RefreshTokenRevokeView(APIView):
    ...
    def post(self, request, token_id):
        try:
            ...
            return Response({"error": "Refresh token not found", "code": "NOT_FOUND"}, status=status.HTTP_404_NOT_FOUND)

        if token.is_revoked:
            return Response(
                {"error": "Token already revoked", "code": "ALREADY_REVOKED"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    examples=[
        OpenApiExample(
            ...
            value=[
                {
                    "id": "sess_123456",
                    "device_info": {"platform": "iOS", "browser": "Safari", "device": "iPhone 14"},
...
@extend_schema(
    ...
    examples=[
        OpenApiExample(response_only=True, name="revoke_session_success", summary="Session révoquée", value=None),
        OpenApiExample(
            response_only=True,
            name="cannot_revoke_current",
            summary="Impossible de révoquer session actuelle",
            value={"error": "Cannot revoke current session", "code": "CANNOT_REVOKE_CURRENT"},
        ),
    ],
)
def revoke_session(request, session_id):
    ...
    if not success:
        if error == "SESSION_NOT_FOUND":
            return Response(
                {"error": "Session not found", "code": "SESSION_NOT_FOUND"}, status=status.HTTP_404_NOT_FOUND
            )
        elif error == "CANNOT_REVOKE_CURRENT":
            return Response(
                {"error": "Cannot revoke current session", "code": "CANNOT_REVOKE_CURRENT"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        else:
            return Response({"error": error, "code": "REVOKE_FAILED"}, status=status.HTTP_400_BAD_REQUEST)
...
@extend_schema(
    ...
    request=inline_serializer(
        ...
        fields={"confirmation": serializers.CharField(help_text='Texte de confirmation "REVOKE ALL"')},
    ),
    responses={
        200: {"type": "object", "properties": {"message": {"type": "string"}, "revoked_count": {"type": "integer"}}},
        400: {"type": "object", "properties": {"error": {"type": "string"}, "code": {"type": "string"}}},
    },
    examples=[
        OpenApiExample(
            ...
            value={"confirmation": "REVOKE ALL"},
        ),
        OpenApiExample(
            response_only=True,
            name="confirmation_required",
            summary="Confirmation requise",
            value={"error": "Confirmation required", "code": "CONFIRMATION_REQUIRED"},
        ),
    ],
)
def revoke_all_sessions(request):
    ...
    if confirmation != "REVOKE ALL":
        return Response(
            {"error": "Confirmation required", "code": "CONFIRMATION_REQUIRED"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    return Response({"message": f"{revoked_count} sessions revoked successfully", "revoked_count": revoked_count})
...
@extend_schema(
    ...
    responses={
        200: {"type": "object", "properties": {"message": {"type": "string"}, "sessions_revoked": {"type": "integer"}}},
        404: {"type": "object", "properties": {"error": {"type": "string"}, "code": {"type": "string"}}},
    },
    examples=[OpenApiExample(response_only=True, name="revoke_device_success", summary="Device révoqué", value=None)],
)
def revoke_device(request, device_id):
    ...
    if not success:
        if error == "DEVICE_NOT_FOUND":
            return Response({"error": "Device not found", "code": "DEVICE_NOT_FOUND"}, status=status.HTTP_404_NOT_FOUND)
        else:
            return Response({"error": error, "code": "REVOKE_FAILED"}, status=status.HTTP_400_BAD_REQUEST)

    return Response({"message": "Device revoked successfully", "sessions_revoked": data.get("sessions_revoked", 0)})
