...
from ..device_info import devices_match, get_device_summary
...
class AuthService:
    ...
    def _get_dummy_hash(cls):
        if cls._DUMMY_HASH is None:
            ...
            cls._DUMMY_HASH = bcrypt.hashpw(pre_hash, bcrypt.gensalt(rounds=auth_settings.BCRYPT_ROUNDS)).decode(
                "utf-8"
            )
    ...
    def authenticate_by_email(
        self, email: str, password: str, application: Application, ip_address: str, device_info: str = ""
        ...
        if auth_settings.RATE_LIMITING_ENABLED:
            if LoginAttempt.is_rate_limited(identifier, self.max_login_attempts, self.rate_limit_window_minutes):
                ...
                brute_force_detected.send(
                    sender=self.__class__, user=None, ip_address=ip_address, attempt_count=self.max_login_attempts
                )
    ...
    def _complete_authentication(
        ...
        if not user.check_password(password):
            ...
            if auth_settings.ACCOUNT_LOCKOUT_ENABLED:
                ...
                if recent_failures >= self.max_login_attempts:
                    ...
                    brute_force_detected.send(
                        sender=self.__class__, user=user, ip_address=ip_address, attempt_count=recent_failures
                    )
        ...
        if device_info:
            ...
            if getattr(is_new, "is_new_device", False) or is_new is True:
                ...
                suspicious_login_detected.send(
                    sender=self.__class__, user=user, ip_address=ip_address, reason="new_device"
                )
        ...
        self._audit_log(
            ...
            ip_address,
            application,
            {"device": get_device_summary(device_info)} if device_info else None,
    ...
    def refresh_access_token(
        self, refresh_token_str: str, application: Application, ip_address: str = None
    ) -> Tuple[bool, Optional[Dict[str, Any]], str]:
        """
        ...
        try:
            ...
            refresh_token = RefreshToken.objects.select_related("user").get(
                pk=refresh_token.pk, application=application
            )
        ...
        if auth_settings.REFRESH_TOKEN_ROTATION:
            ...
            new_refresh_token = RefreshToken.generate(
                user=user, application=application, ip_address=ip_address, device_info=refresh_token.device_info
            )
    ...
    def logout(
        self, refresh_token_str: str, access_token: str = None, ip_address: str = None, application=None
    ) -> bool:
        """
    ...
    def logout_all_devices(self, user: User, access_token: str = None, ip_address: str = None, application=None) -> int:
        """
        Révoque tous les refresh tokens d'un utilisateur.
        Optionnellement blacklist l'access token courant.
        """
        count = RefreshToken.objects.filter(user=user, is_revoked=False).update(is_revoked=True)
    ...
    def register_user(
        ...
        if phone_country_code and phone_number:
            if User.objects.filter(phone_country_code=phone_country_code, phone_number=phone_number).exists():
        ...
        if org_settings.ORGANIZATIONS_ENABLED and getattr(org_settings, "CREATE_DEFAULT_ORGANIZATION", True):
            try:
                ...
                org_service.create_organization(
                    name=org_name, created_by=user, description=f"Default workspace for {user.email or user.full_phone}"
                )
            except Exception as e:
                import logging

                logging.getLogger("tenxyte").error(f"Failed to create default organization for user {user.id}: {e}")
    ...
    def generate_tokens_for_user(
        self, user: User, application: Application, ip_address: str = None, device_info: str = ""
    ) -> Dict[str, Any]:
        """
        ...
        self._audit_log(
            ...
            application,
            (
                {"device": get_device_summary(device_info), "method": "post_registration"}
                if device_info
                else {"method": "post_registration"}
            ),
            device_info=device_info,
        )
    ...
    def change_password(
        self, user: User, old_password: str, new_password: str, ip_address: str = None, application=None
    ) -> Tuple[bool, str]:
        """
    ...
    def _enforce_session_limit(
        self, user: User, application, ip_address: str, device_info: str = ""
        ...
        if active_sessions >= max_sessions:
            ...
            zombies = RefreshToken.objects.filter(user=user, is_revoked=False, expires_at__lte=timezone.now()).update(
                is_revoked=True
            )
            ...
            if action == "deny":
                ...
                self._audit_log(
                    ...
                    ip_address,
                    application,
                    {"action": "denied", "max_sessions": max_sessions, "active_sessions": active_sessions},
                    device_info=device_info,
                )
                ...
                self._audit_log(
                    ...
                    ip_address,
                    application,
                    {"action": "revoked_oldest", "max_sessions": max_sessions, "revoked_count": revoked_count},
                    device_info=device_info,
                )
    ...
    def _enforce_device_limit(
        ...
        if active_devices >= max_devices:
            ...
            if action == "deny":
                ...
                oldest_token = (
                    RefreshToken.objects.filter(user=user, is_revoked=False, expires_at__gt=timezone.now())
                    .order_by("created_at")
                    .first()
                )
                ...
                if oldest_token:
                    ...
                    if oldest_di:
                        ...
                        all_tokens = RefreshToken.objects.filter(user=user, is_revoked=False).exclude(device_info="")
    ...
    def _check_new_device_alert(self, user: User, device_info: str, ip_address: str, application=None) -> None:
        """
        ...
        if not is_known:
            ...
            self._audit_log(
                ...
                ip_address,
                application,
                {"device": summary, "device_info": device_info},
                device_info=device_info,
            )
            ...
            if user.email:
                try:
                    ...
                    logging.getLogger("tenxyte").warning(f"[DeviceAlert] Failed to send new device alert email: {e}")
