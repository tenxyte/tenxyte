...
class PermissionSerializer(serializers.ModelSerializer):
    ...
    parent_code = serializers.CharField(
        write_only=True, required=False, allow_null=True, help_text="Code de la permission parente (hiérarchie)"
    )
...
class ManageRolePermissionsSerializer(serializers.Serializer):
    permission_codes = serializers.ListField(
        child=serializers.CharField(), min_length=1, help_text="Liste des codes de permissions à ajouter ou retirer"
    )
