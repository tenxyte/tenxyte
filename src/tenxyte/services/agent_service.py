...
class AgentTokenService:
            ...
            if organization:
                ...
                if not triggered_by.has_org_permission(organization, perm_code):
                    raise PermissionDenied(
                        f"User does not have permission '{perm_code}' in the specified organization."
                    )
            else:
    ...
    def revoke_all_for_user(self, user) -> int:
        ...
        updated_count = AgentToken.objects.filter(triggered_by=user, status=AgentToken.Status.ACTIVE).update(
    ...
    def revoke_all_for_agent(self, agent_id, organization=None) -> int:
        ...
        query = AgentToken.objects.filter(agent_id=agent_id, status=AgentToken.Status.ACTIVE)
        if organization:
    ...
    def check_circuit_breaker(self, agent_token) -> tuple[bool, str | None]:
        ...
        if agent_token.budget_limit_usd and agent_token.current_spend_usd >= agent_token.budget_limit_usd:
            self.suspend(
                agent_token, AgentToken.SuspendedReason.BUDGET_EXCEEDED
            )  # Note: BUDGET_EXCEEDED doesn't exist natively. ANOMALY can work or extending.
    ...
    def confirm_pending_action(self, confirmation_token, confirmed_by=None) -> AgentPendingAction | None:
        ...
        try:
            action = AgentPendingAction.objects.get(
                confirmation_token=confirmation_token, confirmed_at__isnull=True, denied_at__isnull=True
            )
    ...
    def report_usage(
        self, agent_token, cost_usd: float = 0.0, prompt_tokens: int = 0, completion_tokens: int = 0
    ) -> bool:
        """
