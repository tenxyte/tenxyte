...
APPLE_SETTINGS_OVERRIDE = {
    "APPLE_CLIENT_ID": "com.example.app.signin",
    "APPLE_TEAM_ID": "TEAMID1234",
    "APPLE_KEY_ID": "KEYID98765",
    "APPLE_PRIVATE_KEY": None,  # filled in per-test with a generated EC test key when needed
}
...
class TestSocialConnectionModel:
    ...
    def test_different_providers_same_user(self):
        ...
        SocialConnection.get_or_create_for_user(
            user=user,
            provider="github",
            provider_user_id="gh_multi",
        )
        SocialConnection.get_or_create_for_user(
            user=user,
            provider="google",
            provider_user_id="google_multi",
        )
...
class TestProviderRegistry:
    ...
    @override_settings(TENXYTE_SOCIAL_PROVIDERS=["google", "github", "microsoft", "facebook", "apple"])
    def test_get_provider_returns_apple_instance(self):
        assert isinstance(get_provider("apple"), AppleOAuthProvider)
...
class TestAppleOAuthProvider:
    """Tests for Sign in with Apple (AppleOAuthProvider). See specs/z_aud_1 (Requirement 5)."""

    def test_provider_name(self):
        assert AppleOAuthProvider().provider_name == "apple"

    def test_generate_client_secret_missing_config_returns_none(self):
        provider = AppleOAuthProvider()
        with override_settings(APPLE_CLIENT_ID="", APPLE_TEAM_ID="", APPLE_KEY_ID="", APPLE_PRIVATE_KEY=""):
            assert provider._generate_client_secret() is None

    def test_generate_client_secret_well_formed(self):
        """Header (alg=ES256, kid) et claims exacts (iss/sub/aud/exp<=6 mois) avec une clé EC de test."""
        import jwt as pyjwt
    ...
    def test_generate_client_secret_ephemeral_two_calls_differ(self):
        ...
        with override_settings(
            ...
            time.sleep(1.1)  # ensure iat (second resolution) differs
            secret2 = provider._generate_client_secret()

        assert secret1 is not None and secret2 is not None
        assert secret1 != secret2

    def test_exchange_code_returns_none_when_secret_generation_fails(self):
        provider = AppleOAuthProvider()
        with override_settings(APPLE_CLIENT_ID="", APPLE_TEAM_ID="", APPLE_KEY_ID="", APPLE_PRIVATE_KEY=""):
            assert provider.exchange_code("code123", "https://app.com/callback") is None

    def test_exchange_code_success(self):
        provider = AppleOAuthProvider()
        with (
            patch.object(provider, "_generate_client_secret", return_value="fake.client.secret"),
            patch.object(provider, "_post", return_value={"access_token": "tok", "id_token": "idtok"}) as mock_post,
        ):
            result = provider.exchange_code("code123", "https://app.com/callback")
        assert result == {"access_token": "tok", "id_token": "idtok"}
        sent_data = mock_post.call_args.kwargs.get("data") or mock_post.call_args.args[1]
        assert sent_data["client_secret"] == "fake.client.secret"
    ...
    def test_verify_id_token_success(self):
        ...
        with (
            ...
            patch("jwt.PyJWKClient.get_signing_key_from_jwt", return_value=MagicMock(key="fake_key")),
            patch("jwt.decode", return_value=mock_claims),
        ):
            result = provider.verify_id_token("fake.id.token")
        assert result["provider_user_id"] == "apple_sub_123"
        assert result["email"] == "private@privaterelay.appleid.com"
        assert result["email_verified"] is True

    def test_verify_id_token_signature_failure_returns_none(self):
        provider = AppleOAuthProvider()
        with patch("jwt.PyJWKClient.get_signing_key_from_jwt", side_effect=Exception("bad signature")):
            assert provider.verify_id_token("forged.token") is None

    def test_verify_id_token_jwks_unreachable_fails_closed(self):
        provider = AppleOAuthProvider()
        with patch("jwt.PyJWKClient.get_signing_key_from_jwt", side_effect=Exception("JWKS unreachable")):
            assert provider.verify_id_token("any.token") is None

    def test_get_user_info_returns_none(self):
        provider = AppleOAuthProvider()
        assert provider.get_user_info("irrelevant_access_token") is None

    def test_normalize_email_verified_variants(self):
        provider = AppleOAuthProvider()
        assert provider._normalize({"sub": "1", "email": "a@b.com", "email_verified": "true"})["email_verified"] is True
        assert (
            provider._normalize({"sub": "1", "email": "a@b.com", "email_verified": "false"})["email_verified"] is False
        )
        assert provider._normalize({"sub": "1", "email": "a@b.com", "email_verified": True})["email_verified"] is True
        assert provider._normalize({"sub": "1", "email": "a@b.com", "email_verified": False})["email_verified"] is False
        assert provider._normalize({"sub": "1", "email": "a@b.com"})["email_verified"] is False

    def test_normalize_returns_contract_keys(self):
        provider = AppleOAuthProvider()
        result = provider._normalize({"sub": "x", "email": "x@example.com", "email_verified": True})
...
class TestSocialAuthService:
    ...
    def test_authenticate_creates_social_connection(self):
        ...
        assert SocialConnection.objects.filter(provider="microsoft", provider_user_id="ms_789").exists()
    ...
    def test_authenticate_no_email_no_connection(self):
        ...
        service = SocialAuthService()
        user_data = {"provider_user_id": "no_email_123", "email": None, "first_name": "No", "last_name": "Email"}
...
class TestSocialAuthView:
    ...
    def test_successful_auth_with_access_token(self):
        app = _app("ViewApp5")
        with patch(
            "tenxyte.services.social_auth_service.GitHubOAuthProvider.get_user_info", return_value=GITHUB_USER_DATA
        ):
    ...
    def test_successful_auth_with_code(self):
        ...
        with (
            ...
            resp = _post("github", {"code": "auth_code", "redirect_uri": "https://app.com/callback"}, app=app)
        assert resp.status_code == 200
        assert "access_token" in resp.data
    ...
    def test_code_exchange_failure_returns_401(self):
        ...
        with patch("tenxyte.services.social_auth_service.GitHubOAuthProvider.exchange_code", return_value=None):
            resp = _post("github", {"code": "bad_code", "redirect_uri": "https://app.com/callback"}, app=app)
        assert resp.status_code == 401
    ...
    def test_authenticate_failure_returns_401(self):
        ...
        with patch(
            ...
            with patch(
                "tenxyte.services.social_auth_service.SocialAuthService.authenticate",
                return_value=(False, None, "Some error"),
            ):
                resp = _post("github", {"access_token": "valid_token"}, app=app)
        assert resp.status_code == 401
    ...
    def test_unsupported_provider_lists_apple(self):
        app = _app("ViewAppApple0")
        resp = _post("twitter", {"access_token": "tok"}, app=app)
        assert resp.status_code == 400
        assert "apple" in resp.data["supported_providers"]

    def test_apple_id_token_auth_first_login_uses_user_payload(self):
        """Property 8: First_Auth_User_Payload present → first_name/last_name populated."""
        app = _app("ViewAppApple1")
        with patch(
            "tenxyte.services.social_auth_service.AppleOAuthProvider.verify_id_token",
            return_value={**APPLE_USER_DATA, "provider_user_id": "apple_first_login"},
        ):
            resp = _post(
                "apple",
                {
                    "id_token": "valid_apple_id_token",
                    "user": {"name": {"firstName": "Cupertino", "lastName": "Fruit"}},
                },
                app=app,
            )
        assert resp.status_code == 200
        assert resp.data["user"]["first_name"] == "Cupertino"
        assert resp.data["user"]["last_name"] == "Fruit"

    def test_apple_id_token_auth_subsequent_login_no_user_payload(self):
        """Property 8: First_Auth_User_Payload absent → empty names, authentication still succeeds."""
        app = _app("ViewAppApple2")
        with patch(
            "tenxyte.services.social_auth_service.AppleOAuthProvider.verify_id_token",
            return_value={
                **APPLE_USER_DATA,
                "provider_user_id": "apple_returning",
                "email": "returning_apple@example.com",
            },
        ):
            resp = _post("apple", {"id_token": "valid_apple_id_token"}, app=app)
        assert resp.status_code == 200
        assert resp.data["user"]["first_name"] == ""
        assert resp.data["user"]["last_name"] == ""

    def test_apple_code_flow_uses_verify_id_token_not_get_user_info(self):
        app = _app("ViewAppApple3")
        with (
            patch(
                "tenxyte.services.social_auth_service.AppleOAuthProvider.exchange_code",
                return_value={"access_token": "irrelevant", "id_token": "apple_id_tok"},
            ),
            patch(
                "tenxyte.services.social_auth_service.AppleOAuthProvider.verify_id_token",
                return_value={**APPLE_USER_DATA, "provider_user_id": "apple_code_flow"},
            ) as mock_verify,
            patch(
                "tenxyte.services.social_auth_service.AppleOAuthProvider.get_user_info",
            ) as mock_userinfo,
        ):
            resp = _post("apple", {"code": "auth_code", "redirect_uri": "https://app.com/callback"}, app=app)
        assert resp.status_code == 200
        mock_verify.assert_called_once_with("apple_id_tok")
        mock_userinfo.assert_not_called()

    def test_apple_code_exchange_failure_returns_401(self):
        app = _app("ViewAppApple4")
        with patch("tenxyte.services.social_auth_service.AppleOAuthProvider.exchange_code", return_value=None):
            resp = _post("apple", {"code": "bad_code", "redirect_uri": "https://app.com/callback"}, app=app)
        assert resp.status_code == 401
        assert resp.data["code"] == "CODE_EXCHANGE_FAILED" or resp.data["code"] == "PROVIDER_AUTH_FAILED"

    def test_apple_forged_id_token_returns_401(self):
        """Property 6: id_token invalide → PROVIDER_AUTH_FAILED, aucun utilisateur créé."""
        app = _app("ViewAppApple5")
        with patch("tenxyte.services.social_auth_service.AppleOAuthProvider.verify_id_token", return_value=None):
            resp = _post("apple", {"id_token": "forged_id_token"}, app=app)
        assert resp.status_code == 401
        assert resp.data["code"] == "PROVIDER_AUTH_FAILED"
        assert not User.objects.filter(email="apple@example.com").exists()

    def test_apple_unverified_email_existing_account_refused(self):
        """Property 10: email Apple non vérifié + compte local existant → refus F-03."""
        app = _app("ViewAppApple6")
        _user("f03_apple@example.com")
        with patch(
            "tenxyte.services.social_auth_service.AppleOAuthProvider.verify_id_token",
            return_value={
                "provider_user_id": "apple_f03",
                "email": "f03_apple@example.com",
            ...
            resp = _post("apple", {"id_token": "unverified_apple_id_token"}, app=app)
        assert resp.status_code == 401
        assert resp.data["code"] == "SOCIAL_AUTH_FAILED"
        assert not SocialConnection.objects.filter(provider="apple", provider_user_id="apple_f03").exists()

    def test_apple_private_relay_email_accepted(self):
        app = _app("ViewAppApple7")
        with patch(
            "tenxyte.services.social_auth_service.AppleOAuthProvider.verify_id_token",
            return_value={
                "provider_user_id": "apple_relay",
            ...
            resp = _post("apple", {"id_token": "relay_id_token"}, app=app)
        assert resp.status_code == 200
        assert resp.data["user"]["email"] == "xyz123@privaterelay.appleid.com"
...
class TestSocialAuthCallbackView:
    ...
    def test_authenticate_failure_returns_401(self):
        ...
        with (
            ...
            patch(
                "tenxyte.services.social_auth_service.SocialAuthService.authenticate",
                return_value=(False, None, "Auth error"),
            ),
        ):
    ...
    def test_successful_callback_returns_200(self):
        ...
        with (
            ...
            patch(
                "tenxyte.services.social_auth_service.SocialAuthService.authenticate",
                return_value=(True, {"access": "jwt", "user": {}}, ""),
            ),
        ):
            resp = self._get("github", {"code": "abc", "redirect_uri": "http://app.com"}, app=app)
        assert resp.status_code == 200
    ...
    def test_exception_handling_returns_400(self):
        with patch(
            "tenxyte.services.social_auth_service.GitHubOAuthProvider.exchange_code", side_effect=Exception("Boom")
        ):
            resp = self._get("github", {"code": "abc", "redirect_uri": "http://app.com"})
        assert resp.status_code == 400
