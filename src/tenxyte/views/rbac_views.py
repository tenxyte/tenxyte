        ...
        examples=[
            ...
            OpenApiExample(
                ...
                value={
                    ...
                    "description": "Permet de voir les rapports de toutes les organisations.",
                    "parent": None,
                    "children": [{"id": "10", "code": "reports.export", "name": "Exporter les rapports"}],
                    "created_at": "2024-01-20T10:00:00Z",
                },
            ),
            OpenApiExample(
                ...
                response_only=True,
                status_codes=["404"],
                value={"error": "Permission not found", "code": "NOT_FOUND"},
            ),
        ],
    ),
...
class PermissionDetailView(APIView):
    ...
    def get(self, request, permission_id):
        try:
            ...
            return Response({"error": "Permission not found", "code": "NOT_FOUND"}, status=status.HTTP_404_NOT_FOUND)
            ...
            OpenApiExample(
                ...
                value={
                    ...
                    "permissions": [
                        ...
                        {"id": "12", "code": "logs.view", "name": "Consulter les logs"},
                    ],
                    "is_default": False,
                    "created_at": "2024-01-20T10:00:00Z",
    ...
    put=extend_schema(
        ...
        examples=[
            ...
            OpenApiExample(
                ...
                value={
                    ...
                    "permissions": [
                        {"id": "5", "code": "users.view", "name": "Lister les utilisateurs"},
                        {"id": "12", "code": "logs.view", "name": "Consulter les logs"},
                        {"id": "13", "code": "logs.export", "name": "Exporter les logs"},
                    ],
                    "is_default": False,
                    "created_at": "2024-01-20T10:00:00Z",
            ...
            OpenApiExample(
                ...
                response_only=True,
                status_codes=["404"],
                value={"error": "Role not found", "code": "NOT_FOUND"},
            ),
        ],
    ),
    delete=extend_schema(
...
class RoleDetailView(APIView):
    ...
    def get(self, request, role_id):
        try:
            ...
            return Response({"error": "Role not found", "code": "NOT_FOUND"}, status=status.HTTP_404_NOT_FOUND)
    ...
    post=extend_schema(
        ...
        examples=[
            OpenApiExample(
                ...
                request_only=True,
                value={"permission_codes": ["users.create", "users.update"]},
            ),
            OpenApiExample(
...
class RolePermissionsView(APIView):
    ...
    def post(self, request, role_id):
        ...
        if not_found:
            return Response(
                {"error": "Some permissions not found", "code": "PERMISSIONS_NOT_FOUND", "not_found": list(not_found)},
                status=status.HTTP_400_BAD_REQUEST,
            )

        existing_codes = set(role.permissions.filter(code__in=permission_codes).values_list("code", flat=True))
    ...
    def delete(self, request, role_id):
        ...
        assigned_codes = set(role.permissions.filter(code__in=permission_codes).values_list("code", flat=True))
    ...
    post=extend_schema(
        ...
        examples=[
            ...
            OpenApiExample(
                ...
                response_only=True,
                status_codes=["200"],
                value={"message": "Role assigned", "roles": ["tech_support", "default_user"]},
            ),
            OpenApiExample(
                name="role_not_found",
                summary="Rôle introuvable",
                response_only=True,
                status_codes=["404"],
                value={"error": "Role not found", "code": "ROLE_NOT_FOUND"},
            ),
        ],
    ),
    delete=extend_schema(
...
class UserRolesView(APIView):
    ...
    def get(self, request, user_id):
        try:
            ...
            return Response({"user_id": str(user.id), "roles": RoleListSerializer(roles, many=True).data})
    ...
    def post(self, request, user_id):
        ...
        if user.assign_role(serializer.validated_data["role_code"]):
            return Response({"message": "Role assigned", "roles": user.get_all_roles()})
        else:
            return Response({"error": "Role not found", "code": "ROLE_NOT_FOUND"}, status=status.HTTP_404_NOT_FOUND)
    ...
    def delete(self, request, user_id):
        ...
        if not role_code:
            return Response(
                {"error": "role_code query parameter required", "code": "MISSING_PARAM"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if user.remove_role(role_code):
            return Response({"message": "Role removed", "roles": user.get_all_roles()})
        else:
            return Response({"error": "Role not found", "code": "ROLE_NOT_FOUND"}, status=status.HTTP_404_NOT_FOUND)
    ...
    post=extend_schema(
        ...
        examples=[
            OpenApiExample(
                ...
                request_only=True,
                value={"permission_codes": ["users.view", "reports.export"]},
            ),
            OpenApiExample(
            ...
            OpenApiExample(
                ...
                response_only=True,
                status_codes=["404"],
                value={"error": "User not found", "code": "NOT_FOUND"},
            ),
        ],
    ),
    delete=extend_schema(
...
class UserDirectPermissionsView(APIView):
        ...
        return Response(
            {
                "user_id": str(user.id),
                "email": user.email,
                "direct_permissions": PermissionSerializer(user.direct_permissions.all(), many=True).data,
    ...
    def post(self, request, user_id):
        ...
        existing_codes = set(user.direct_permissions.filter(code__in=permission_codes).values_list("code", flat=True))
        new_permissions = [p for p in permissions if p.code not in existing_codes]
    ...
    def delete(self, request, user_id):
        ...
        assigned_codes = set(user.direct_permissions.filter(code__in=permission_codes).values_list("code", flat=True))
        to_remove = [p for p in permissions if p.code in assigned_codes]
        not_removed = [p.code for p in permissions if p.code not in assigned_codes]
