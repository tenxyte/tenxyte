...
- [x] 1. Apple Sign-In : provider et settings
  - [x] 1.1 Ajouter les settings Apple à `SocialSettingsMixin`
  ...
  - [x] 1.2 Implémenter `AppleOAuthProvider`
  ...
  - [x] 1.3 Câbler le provider dans la vue sociale et l'orchestrateur
  ...
  - [x] 1.4 Write unit tests for the Apple client secret and settings
  ...
  - [x] 1.5 Write property test for well-formed ephemeral Apple client secret
    - **Property 5: Le client secret Apple est un JWT ES256 bien formé et éphémère**
    - **Validates: Requirements 5.2**

  - [x] 1.6 Write property test for fail-closed id_token validation
  ...
  - [x] 1.7 Write property test for normalized Apple user dict
    - **Property 7: Normalisation du dict utilisateur Apple** (email_verified multi-types,
      private relay, sub aléatoires)
    - **Validates: Requirements 5.6**

  - [x] 1.8 Write property test for optional first-auth user payload
    - **Property 8: Le payload de première autorisation est optionnel**
    - **Validates: Requirements 5.7**

  - [x] 1.9 Write unit tests for existing-provider non-regression and F-03 on Apple
  ...
  - [x] 1.10 Documenter le provider Apple
...
- [x] 2. Checkpoint - Ensure all tests pass
  ...
  - ✅ Full suite green (2585 passed incl. new Apple + Import_Guard tests, 0 unexpected failures;
    fastapi/ tests skipped — missing optional `sqlalchemy` dep in this env, pre-existing/out of
    scope). One pre-existing test updated for the new `SOCIAL_PROVIDERS` default (see tasks.md
    notes / commit).

- [x] 3. Import_Guard (PEP 562)
  - [x] 3.1 Ajouter `TenxyteMissingDependencyError` et refactorer `tenxyte/__init__.py`
  ...
  - [x] 3.2 Write property test for import without Django
  ...
  - [x] 3.3 Write property test for explicit failure on Django-only symbols
    - **Property 2: Accès à un symbole Django-only sans Django échoue explicitement**
    - **Validates: Requirements 2.4**

  - [x] 3.4 Write unit tests for guard transparency with Django installed
  ...
  - [x] 3.5 Write the public-API snapshot test
...
- [x] 4. Inversion du packaging
  - [x] 4.1 Inverser les extras dans `pyproject.toml`
  ...
  - [x] 4.4 Rédiger la section « 0.9 → 1.0 » du guide de migration
  ...
  - [x] 4.5 Préparer l'ultime release 0.9.x d'avertissement
...
- [x] 5. Checkpoint - Ensure all tests pass
  ...
  - ✅ Full suite green after packaging inversion (see task 10.1 for the final run).

- [ ] 6. Politique de sécurité et divulgation
  - [x] 6.1 Rédiger `SECURITY.md`
...
- [ ] 7. Signature et attestation des releases
  - [x] 7.1 Migrer `publish.yml` vers Trusted Publishing + attestations
  ...
  - [x] 7.3 Documenter la procédure de release et de vérification
...
- [x] 8. Contrat de stabilité et dossier d'audit
  - [x] 8.1 Rédiger `docs/en/stability.md` + `docs/fr/stability.md`
  ...
  - [x] 8.2 Rédiger le modèle de menaces
  ...
  - [x] 8.3 Rédiger le périmètre d'audit
  ...
  - [x] 8.4 Rédiger la checklist pré-audit ASVS
...
- [x] 9. Release engineering 1.0
  - [x] 9.1 Basculer la version et le classifieur
    - `pyproject.toml` : `version = "1.0.0"`, classifieur `5 - Production/Stable` ; test
      unitaire de cohérence version/classifieur
    - _Requirements: 7.1_

  - [x] 9.2 CHANGELOG 1.0.0 et mise à jour des READMEs
  ...
  - [x] 9.3 Vérifier l'invariance du schéma
...
- [ ] 10. Non-régression finale et validation manuelle
  - [x] 10.1 Exécuter la suite de tests existante complète
    - L'intégralité de `tests/` passe sans modification d'aucun test existant
    - _Requirements: 8.4_
    - ✅ `pytest tests/ --ignore=tests/integration/fastapi` : **2647 passed, 0 failed, 8 skipped**
      (2585 pré-existants + ~62 nouveaux pour cette phase). `tests/integration/fastapi/` ignoré
      (dépendance optionnelle `sqlalchemy` absente de cet environnement — hors périmètre Phase 1,
      non lié à ce travail). 2 tests pré-existants mis à jour car leur assertion encodait
      littéralement l'ancien défaut `SOCIAL_PROVIDERS` (changement requis par 5.9, pas une
      régression) : `tests/integration/django/unit/test_secure_mode.py`.
  ...
  - [ ] 10.2 Dérouler la campagne de tests manuels
    ...
    - ⚠️ Hors de portée de l'agent (comptes/droits externes requis : Apple Developer, admin
      GitHub, owner PyPI). Registre `manual_tests.md` mis à jour avec le statut réel et les
      contreparties automatisées déjà vertes pour chaque item (MT-1, MT-7 partiellement couverts).
...
- [ ] 11. Checkpoint final - Ensure all tests pass
  ...
  - ✅ Tests : voir 10.1. ⬜ Registre manuel : voir 10.2 — **v1.0.0 non taguée, en attente de
    validation utilisateur et des tests manuels MT-1 à MT-7.**
