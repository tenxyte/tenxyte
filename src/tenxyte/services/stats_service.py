...
from ..models import (
    get_user_model, AuditLog, BlacklistedToken,
...
class StatsService:
    ...
    def _login_by_method(self, since):
        ...
        methods = AuditLog.objects.filter(
            action='login', created_at__gte=since
        ).values('details__method').annotate(
            count=Count('id')
        )
        result = {}
        ...
        if not result:
            result['total'] = AuditLog.objects.filter(
                action='login', created_at__gte=since
            ).count()
        return result
    ...
    def _token_stats(self, today):
        ...
        return {
            ...
            'tokens_refreshed_today': AuditLog.objects.filter(
                action='token_refresh', created_at__gte=today
            ).count(),
            'tokens_revoked_today': RefreshToken.objects.filter(
                is_revoked=True, last_used_at__gte=today
            ).count(),
        }

    def _top_failure_reasons(self, since, limit=5):
        reasons = LoginAttempt.objects.filter(
    ...
    def _logins_per_day(self, days):
        ...
        data = LoginAttempt.objects.filter(
    ...
    def _auth_summary(self):
        ...
        return {
            'total_logins_today': LoginAttempt.objects.filter(
                success=True, created_at__gte=today
            ).count(),
            'failed_logins_today': LoginAttempt.objects.filter(
                success=False, created_at__gte=today
            ).count(),
            'active_refresh_tokens': RefreshToken.objects.filter(
                is_revoked=False, expires_at__gt=now
            ).count(),
    ...
    def _blacklisted_token_stats(self, now):
        by_reason = BlacklistedToken.objects.filter(
            expires_at__gt=now
        ...
        return {
            ...
            'expired_pending_cleanup': BlacklistedToken.objects.filter(
                expires_at__lt=now
            ).count(),
        }
    ...
    def get_gdpr_stats(self):
        ...
        by_status = AccountDeletionRequest.objects.values('status').annotate(
            count=Count('id')
        )
        ...
        return {
            ...
            'data_exports': {
                'total_today': AuditLog.objects.filter(
                    action='data_exported', created_at__gte=today
                ).count(),
                'total_this_month': AuditLog.objects.filter(
                    action='data_exported', created_at__gte=month_ago
                ).count(),
            },
        }
    ...
    def get_organization_stats(self):
        ...
        by_role = OrganizationMembership.objects.values('role__name').annotate(
            count=Count('id')
        )
