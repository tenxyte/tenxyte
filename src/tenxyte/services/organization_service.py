...
class OrganizationService:
    ...
    def create_organization(
        ...
        try:
            ...
            self.OrganizationMembership.objects.create(
                user=created_by, organization=organization, role=owner_role, status="active"
            )
    ...
    def update_organization(self, organization, user, **updates) -> Tuple[bool, str]:
        ...
        # Audit log
        self._audit_log("organization_updated", user, {"organization_id": organization.id, "updates": updates})
    ...
    def delete_organization(self, organization, user) -> Tuple[bool, str]:
        ...
        # Audit log
        self._audit_log(
            "organization_deleted", user, {"organization_id": organization.id, "organization_name": organization.name}
        )
    ...
    def add_member(
        ...
        if self.OrganizationMembership.objects.filter(user=user_to_add, organization=organization).exists():
        ...
        self._audit_log(
            "member_added",
            added_by,
            {"organization_id": organization.id, "user_id": user_to_add.id, "role_code": role_code},
        )
    ...
    def remove_member(self, organization, user_to_remove, removed_by) -> Tuple[bool, str]:
        ...
        # Audit log
        self._audit_log(
            "member_removed", removed_by, {"organization_id": organization.id, "user_id": user_to_remove.id}
        )
    ...
    def create_invitation(
        ...
        OrganizationInvitation.objects.filter(organization=organization, email=email, status="pending").update(
            status="expired"
        )

        # Create invitation
        invitation = OrganizationInvitation.create_invitation(
            organization=organization, email=email, role=role, invited_by=invited_by, expires_in_days=expires_in_days
        )
