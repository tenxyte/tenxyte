    ...
    @extend_schema(
        ...
        examples=[
            OpenApiExample(
                ...
                value={"status": "pending", "grace_period_expiring": True},
            ),
            OpenApiExample(
                name="user_specific",
                summary="Demandes utilisateur spécifique",
                value={"user_id": 12345, "ordering": "-requested_at"},
            ),
        ],
    )
    @require_permission("gdpr.admin")
    def get(self, request):
        queryset = AccountDeletionRequest.objects.select_related("user", "processed_by").all()
...
class DeletionRequestDetailView(APIView):
    ...
    def get(self, request, request_id):
        try:
            deletion_request = AccountDeletionRequest.objects.select_related("user", "processed_by").get(id=request_id)
        except AccountDeletionRequest.DoesNotExist:
            return Response(
                {"error": "Deletion request not found", "code": "NOT_FOUND"}, status=status.HTTP_404_NOT_FOUND
            )
...
class ProcessDeletionView(APIView):
    ...
    @extend_schema(
        ...
        request=inline_serializer(
            ...
            fields={
                ...
                "admin_notes": serializers.CharField(
                    required=False, allow_blank=True, help_text="Notes administratives optionnelles"
                ),
            },
        ),
        responses={
        ...
        examples=[
            ...
            OpenApiExample(
                ...
                value={"error": "Cannot process unconfirmed deletion request", "code": "REQUEST_NOT_CONFIRMED"},
            ),
        ],
    )
    ...
    def post(self, request, request_id):
        ...
        return Response(
            {"error": "Failed to process deletion", "code": "PROCESSING_FAILED"},
...
class ProcessExpiredDeletionsView(APIView):
    ...
    @extend_schema(
        ...
        examples=[
            OpenApiExample(response_only=True, name="batch_success", summary="Traitement batch réussi", value=None)
        ],
        request=None,
    )
    @require_permission("gdpr.process")
