...
class AccountDeletionService:
    ...
    def request_deletion(
        ...
        if user.is_2fa_enabled:
            ...
            if not otp_code:
                self._audit_log(
                    "deletion_request_failed", user, ip_address, {"reason": "missing_2fa", "user_agent": user_agent}
                )
            ...
            if not otp_service.verify_otp(user, otp_code, otp_type="login_2fa"):
                self._audit_log(
                    ...
                    user,
                    ip_address,
                    {"reason": "invalid_2fa", "otp_code": otp_code[:6] + "..." if len(otp_code) > 6 else otp_code},
                )
        ...
        try:
            deletion_request = AccountDeletionRequest.create_request(
                user=user, ip_address=ip_address, user_agent=user_agent, reason=reason
            )
    ...
    def confirm_deletion(self, token: str, ip_address: str = None) -> Tuple[bool, Optional[Dict[str, Any]], str]:
        ...
        """
        try:
            deletion_request = AccountDeletionRequest.objects.get(confirmation_token=token, status="confirmation_sent")
            ...
            try:
                ...
                self._audit_log(
                    ...
                    deletion_request.user,
                    ip_address,
                    {"request_id": deletion_request.id, "error": str(e)},
                )
    ...
    def cancel_deletion(
        ...
        active_requests = AccountDeletionRequest.objects.filter(
            user=user, status__in=["pending", "confirmation_sent", "confirmed"]
        )
        ...
        self._audit_log("deletion_request_cancelled", user, ip_address, {"cancelled_requests": cancelled_count})

        return (
            True,
            {"message": f"Cancelled {cancelled_count} deletion request(s).", "cancelled_count": cancelled_count},
            "",
        )
    ...
    def get_user_requests(self, user: User) -> Dict[str, Any]:
        ...
        active_request = requests.filter(status__in=["pending", "confirmation_sent", "confirmed"]).first()
            ...
            "active_request": (
                {
                    ...
                    "grace_period_ends_at": (
                        active_request.grace_period_ends_at.isoformat() if active_request.grace_period_ends_at else None
                    ),
                    "days_remaining": (
                        (active_request.grace_period_ends_at - timezone.now()).days
                        if active_request.grace_period_ends_at
                        else None
                    ),
                }
                if active_request
        ...
        for request in expired_requests:
            if request.execute_deletion():
                ...
                self._audit_log(
                    ...
                    request.user,
                    None,
                    {"request_id": request.id, "processed_at": timezone.now().isoformat()},
                )
            ...
            "requests": [
                {
                    ...
                    "reason": req.reason,
                    "days_remaining": (
                        (req.grace_period_ends_at - timezone.now()).days if req.grace_period_ends_at else None
                    ),
                }
                for req in requests
            ],
    ...
    def admin_process_request(
        ...
        if action == "approve":
            ...
            self._audit_log(
                ...
                deletion_request.user,
                None,
                {"request_id": request_id, "admin_id": admin_user.id, "admin_email": admin_user.email},
            )
            ...
            self._audit_log(
                "deletion_request_cancelled_admin",
                deletion_request.user,
                None,
                {"request_id": request_id, "admin_id": admin_user.id, "admin_email": admin_user.email},
            )
            ...
            if success:
                self._audit_log(
                    "deletion_request_executed_admin",
                    deletion_request.user,
                    None,
                    {"request_id": request_id, "admin_id": admin_user.id, "admin_email": admin_user.email},
                )
